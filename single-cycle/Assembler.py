...
# --- Configuración ---
DATA_BASE = 0x00000000  # dirección base para la sección .data
...
def main():
    ...
    # Crear rutas completas para todos los archivos
    input_file = os.path.join(script_dir, "instrucciones.txt")
    output_file = os.path.join(script_dir, "salida_ensamblador.txt")
    binary_file = os.path.join(script_dir, "salida_binario.txt")
    hex_file = os.path.join(script_dir, "salida_hexadecimal.txt")
    ...
    with open(output_file, "w", encoding="utf-8") as f_main, \
        ...
        # Escribir encabezados solo en archivo principal
        ...
        if data_entries:
            f_hex.write("# .DATA SECTION\n")
            
            for entry in data_entries:
                if 'error' not in entry:
                    f_bin.write(f"{entry['binary']}\n")  # SOLO binario puro
                    f_hex.write(f"PC:{entry['pc']:08x} {entry['hex']}\n")
            
            f_hex.write("\n")
        
        # SEGUNDO: Escribir instrucciones .text
        f_hex.write("# .TEXT SECTION\n")
        for instr in text_instructions:
            f_bin.write(f"{instr['binary']}\n")  # SOLO binario puro
            f_hex.write(f"PC:{instr['pc']:08x} {instr['hex']}\n")

    print(f"\n✅ Resultados guardados en:")
    print(f"   📄 Salida completa: '{output_file}'")
    print(f"   🔢 Solo binario (32 bits puros): '{binary_file}'")
